...
$sql = "INSERT INTO `order` (`orderID`, `userID`, `restID`, `STATUS`, `cost`)
VALUES (NULL, '" . $_SESSION['ID'] . "', '1', '0', '" . $cost . "');";
...
for ($i = 0; $i < sizeof($arr); $i++) {
    ...
    if ($count == 1) {
        $sql = 'UPDATE `orderitems` SET quantity = quantity + 1
        WHERE orderID = ' . $orderID . ' AND itemID =' . $arr[$i];
    } else {
        $sql = "INSERT INTO `orderitems` (`orderID`, `itemID`, `quantity`)
        VALUES ('" . $orderID . "', '" . $arr[$i] . "', '1')";
    }
    $result = mysqli_query($connection, $sql) or die(mysqli_error($connection));
}
